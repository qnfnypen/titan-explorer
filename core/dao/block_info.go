...
INSERT INTO %s(device_id, carfile_cid, block_size, blocks, max_created_time, time)
(SELECT device_id, carfile_cid, sum(size) AS block_size, count(*) AS blocks, max(created_time) as max_created_time,
		DATE_FORMAT(
